...
@Inject constructor(val getGitsUseCase: GetGitsUseCase) : BaseViewModel() {
    ...
    val PER_PAGE = 5           // loading each page 20 item
    var canLoadMore = true      // Default can load more
    var isLoading = false       // Track status of loading -> need improve

    fun getGits(position: Int) = getGitsUseCase(GetGitsUseCase.Param(currentPage, position, (currentPage - 1) * PER_PAGE)) { it.either(::handleFailure, ::handleGits) }
